...
import { Plus, Edit, Trash2, Send, Clock, CheckCircle, XCircle } from 'lucide-react';
...
import { UsersDropdown, ALL_USERS_VALUE } from '@features/user/components/UsersDropdown';
...
export default function Notifications() {
  ...
  const handleEdit = (notification: Notification) => {
    ...
    reset({
      ...
      users: notification.users?.map((userId: number) => userId.toString()) || [],
                ...
                {watch('users')?.length > 0 && (
                  <p className="text-sm text-gray-600">
                    {watch('users').includes(ALL_USERS_VALUE) 
                      ...
                      : `${watch('users').length} user${watch('users').length !== 1 ? 's' : ''} selected`
                    }
                  </p>
                )}
              </div>
...
{/* Schedule date and time for the notification. Todo:// uncomment it when supported by our backend */}
              {/* <div className="space-y-2">
              ...
              <DialogFooter>
                ...
                <Button type="submit" disabled={createMutation.isPending || broadcastMutation.isPending}>
                  {(createMutation.isPending || broadcastMutation.isPending) ? 'Sending...' : 'Send Notification'}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <form onSubmit={handleFormSubmit(onSubmit)} className="space-y-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={updateMutation.isPending || broadcastMutation.isPending}>
